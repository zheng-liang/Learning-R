...
```{r adf cpi level}
# Indicate type = "drift" as there was a linear trend in the series based on plot
...
The **`summary`** function returns the full output of the ADF test, but we only need the bottom section where the test statistics and the critical values are. The first test statistic, for `tau2`, is higher than the 5% critical value (-1.0508 > -2.88) but we need it to be lower than the critical value to reject the null. Therefore, the CPI series contains unit root. The other value, for `phi1`, in the test statistic tells us that there is a drift in the time series since it is more than the 5% critical value.

```{r kpss cpi level}
# Indicate type = "mu" for drift

train$CPI %>%
  ur.kpss(type = "mu", lags = "long") %>%
  summary()
```

The KPSS test rejected the null hypothesis (test statistic 1.26 > 5% critical value 0.463), which means there is unit root in the CPI series.

```{r adf dspi level}
# Indicate type = "none" as DSPI looked like a random walk in the plot
...
train$CLI %>%
  ur.kpss(type = "tau", lags = "long") %>%
  summary()
```

We rejected the null in the ADF test, and do not reject the null in the KPSS test. This indicated that CLI is trend stationary, which means it can become stationary after removing the trend from the series (aka de-trending). However, I simply took the first difference to make it stationary.
...
```{r adf dcpi}
# After differencing, there should be no deterministic trend in the plots
...
plot(as.ts(diff.train$d.CPI),
     col = "darkblue", type = "l", lwd = 3,
     ylab = "d.CPI", main = "Actual vs Fitted CPI at First Difference Using ARIMA",
     xlim = c(25, 42))

lines(fitted(arima1), col = "red", type = "l", lwd = 3)

lines(fitted(arima2), col = "gray", type = "l", lwd = 3)

abline(h = arima2$coef[4], col = "black", lwd = 2)

legend(x = "topright", legend = c("Actual CPI", "arima1", "arima2", "Mean from arima2"),
...
legend(x = "bottomleft", legend = c("Actual CPI", "ardl"),
...
## 7.3 Vector Autoregression Model

A VAR model allows us to model the influence of two or more time series on one another. This means that the variables (e.g. $x$ and $y$) are simultaneously determined by the lags of these variables. A VAR(p) model of two variables can be written as:
...
Based on AIC, I would choose to use 2 lag for the VAR model, i.e. VAR(2) model. We could use the SC (or BIC) selected lags for a parsimonious model. The function **`VAR`** in **`vars`** package allows us to estimate this model.
...
The output shows the four equations, one for each variable, with 4 lags of each variable. Therefore, the model gets really big as more variables and lags are added, which can become a problem.
...
2.  r \<= 1: value under test column is more than value under 5pct column (statistically significant) and we have more than one cointegrating relationship.
...
If there is indeed cointegration, the coefficient of $v_{t-1}$ should be negative. This is because, taking the long-run pattern into account, if $y_{t-1}$ was above/below $y^*$, then we should expect $y_t$ to decrease/increase. 
...
Instead of extracting the residuals, we can use the **`uecm`** and **`recm`** functions to get the unrestricted and restricted ECMs. The first model will show all the coefficients for the long-run and short-run variables, while the second will only show the short-run variables with the ECT.
...
We can see that the `r_ecm` output is the same as the `un_ecm` output, less the long-run variables. And as expected, the ECT is negative (and statistically significant).
