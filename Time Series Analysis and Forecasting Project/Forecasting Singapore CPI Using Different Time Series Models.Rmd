...
Causality test for determining whether variables are useful in forecasting other variables. The main models used in this project are the ARIMA, ARDL and VAR models, which
will be discussed in detail in the later sections. However, I would not be tackling the issue of seasonality in this project as it will become a lengthy discussion. Instead, I would do this in a future project as I get familiar with the treatment of seasonal effects on time series and allow for comparison against the results I have documented here.
...
consider adding seasonal dummies to our time series model. However, since seasonality is not the main purpose of this project, we may treat this as a factor for discussion in future research.
...
(i.e. $y_t$ - $y_{t-1}$) or to de-trend a trend stationary series. If a series is stationary after taking the first difference, we now have an I(1) series or we can say that the
variable is integrated of order 1. The original series (if stationary) is I(0), or at
levels.
...
The log-likelihood is much higher and AIC much lower than the `arima2` model estimated in Section 9.1, which should indicate a better fit. However, `d.PCE` seems to be statistically insignificant and there are no lags for this variable. Let's estimate another ARDL model with `d.PCE` and see the results.

```{r ardl2}
ardl2_lag <- ARDL::auto_ardl(formula = d.CPI ~ d.DSPI + d.CLI,
                             data = as.zoo(diff.train),
                             max_order = 4, grid = T,
                             selection = "AIC")

ardl2 <- ardl2_lag$best_model
...
There seems to be negligible difference to the log-likelihood, but AIC value has decreased. As such, I might choose the more parsimonious `ardl2` model but I would still analyze them both on out-of-sample forecasts in the later section.
...
lines(fitted(ardl1), col = "red", type = "l", lwd = 3)

legend(x = "bottomleft", legend = c("Actual CPI", "ardl1"),
...
Based on AIC, I would choose to use 4 lag for the VAR model, i.e. VAR(4) model. We could use the SC (or BIC) selected lags for a parsimonious model. The function **`VAR`** in **`vars`** package allows us to estimate this model.
...
determine true cause-and-effect relationships between variables. It can be a
...
Since we have I(1) variables, there are a few choices presented for
