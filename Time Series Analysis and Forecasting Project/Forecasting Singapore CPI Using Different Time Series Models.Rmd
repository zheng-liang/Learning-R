...
```{r jo test trace}
# Indicate type = "trace" for trace test, "eigne" for eigenvalue test

# Indicate ecdet = "const" to add constant to cointegration, "none" or "trend" for no intercept or to add trend to cointegration
...
2.  r \<= 1: value under test column is more than value under 5pct column (statistically insignificant) and we have more than one cointegrating relationship.
...
lines(ts(fitted(var$varresult$d.CPI), start = c(1978, 4), frequency = 4), col = "green", type = "l", lwd = 3)
...
Two common problems encountered in time series analysis are serial correlation of errors and heteroskedastic errors. These can affect our analysis as estimators are no longer efficient and the usual test statistics become unreliable, which can be important if inference is the main purpose. The Breusch-Godfrey Test using the function **`bgtest`** can be used to test for serial correlation, while the Breusch-Pagan Test using **`bptest`** can be used to test for heteroskedasticity. Both functions can be found in the **`lmtest`** package.
