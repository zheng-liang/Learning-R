...
In this section, I provided more details about each variable and generated univariate and multivariate plots to find patterns in and interaction between the variables. 

### 4.1 Changing Data Format

To visualize the plots, rather than using a data frame object, it is best to convert them into an `xts` object. The conversion requires two arguments. First is the vector or matrix to be converted to xts object and a vector of the date. We have the dates in the data frame but they are not formatted as dates, so I used the **`yearmon`** and **`yearqtr`** functions to format them to the appropriate types.
...
`cpi.xts` contained 738 monthly observations from January 1961 to June 2022. However, I would need to adjust this variable as the Composite Leading Index starts from 1978 and it is a quarterly series. For time series analysis to work, we need the variables to have same time interval.
...
### 4.3 Domestic Supply Price Index
...
Similar to `cpi.xts`, `dspi.xts` 582 monthly observations from January 1974 to June 2022. We would need to adjust the time interval to match the CLI data.
...
## 6 Unit Root Test for Stationarity {.tabset .tabset-pills}
...
I performed the ADF and KPSS test on the three variables in the `data` object, firstly on the variables at levels then at first differences, using the **`ur.df`** and **`ur.kpss`** function in the **`urca`** package.
...
## 7 Model Selection {.tabset .tabset-pills}

In this section, I discussed the different possible time series models that can be created from our data.

### 7.1 Autoregressive (Integrated) Moving Average Model {.tabset}

#### 7.1.1 Brief Introduction to ARIMA Models
...
This can be written simply as ARIMA(p, d, q). In Section 6, I had tested that the CPI series is an I(1) series, which means it is stationary at first difference. Therefore, we have an ARIMA(p, 1, q) model. The p and q lags are determined in various ways.

#### 7.1.2 ACF and PACF

Firstly, it is common to use the autocorrelation and partial autocorrelation function to determine the lag order. The functions **`Acf`** and **`Pacf`** in the **`forecast`** package can plot the ACF and PACF.

```{r acf pacf, fig.align='center', echo=FALSE}
...
#### 7.1.3 ARIMA Based on ACF and PACF
...
### 7.2 Autoregressive Distributed Lag Models {.tabset}

#### 7.2.1 Brief Introduction to ARDL Models
...
#### 7.2.2 ARDL Model Lag Selection
...
### 7.3 Vector Autoregressive Model {.tabset}

#### 7.3.1 Brief Introduction to VAR Models
...
### 9.2 Error Correction Model {.tabset .tabset-pills}

#### 9.2.1 Steps to Estimating ECM
...
### 9.3 Vector Error Correction Model {.tabset .tabset-pills}
...
#### 9.3.1 Estimating VECM Equations
...
## 10 Post-Estimation Tests {.tabset .tabset-pills}
...
## 11 Forecasting and Evaluation {.tabset .tabset-pills}

### 11.1 Forecasting Details
...
Firstly, the forecasted values will be in first-difference. Conversion to levels can be done simply by adding the cumulative forecasted values to the last observation used in the `diff.train` dataset.

Secondly, models such as ARIMA and VAR uses endogenous variable but ARDL models contains exogenous variables. This means that we would need data on the exogenous variables in the ARDL models for the forecasting to work. Such data can be obtained through the use of ARIMA models to forecast exogenous variables before inputting them into the ARDL forecast. However, for simplicity, I would use the data in the `diff.test` set to compare the performance of the ARDL and ARDL-EC models while ARIMA and VAR/VECM would be compared to one another.
...
### 11.2 Forecast Performance of ARIMA Models
