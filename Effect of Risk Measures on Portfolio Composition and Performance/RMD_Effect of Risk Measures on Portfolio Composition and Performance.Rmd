...
Based on the random portfolios generated, `Min_Var` and `Min_SemiDev` portfolios have the same asset weights, which would mean they have the same performance.
...
The minimum risk portfolios all had better cumulative returns than the SPY benchmark, with the `Min_HCVAR` and `Min_HVaR` having similar cumulative returns, followed by `Min_DownDev` and `Min_Var`/`Min_SemiDev` portfolios. In general, the SPY benchmark has larger drawdowns than the minimum risk portfolios.
...
Out of 24,999 random portfolios generated in Section 5, only 795 portfolios meet the target return constraint.
...
With an additional constraint of having 20% target return, the optimal portfolios have similar cumulative returns (since we added a target return). The SPY benchmark still has larger drawdowns compared to the optimal portfolios in general. The `Mean_DownDev` portfolio seems to have larger drawdowns amongst the optimal portfolios, followed by `Mean_VaR` and `Mean_CVaR` portfolios.
