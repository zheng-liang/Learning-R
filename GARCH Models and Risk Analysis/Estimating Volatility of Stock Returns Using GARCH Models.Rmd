...
```{r plot EGARCH(1,1) with t-distribution}
# Plot some data from the EGARCH(1, 1) with t-distribution

par(mfrow = c(2, 2))

# Actual return series with conditional SD
plot(fitEGARCH11_std, which = 1)

# Conditional volatility vs absolute returns
plot(fitEGARCH11_std, which = 3)

# Distribution of residuals
plot(fitEGARCH11_std, which = 8)

# Q-Q plot of residuals to check for normality
plot(fitEGARCH11_std, which = 9)
```
...
In this section, I evaluated the out-of-sample forecast performance of the 12 models created in Section 3. However, before jumping into the forecast, there is a need to highlight the difficulty of evaluating GARCH models. 
