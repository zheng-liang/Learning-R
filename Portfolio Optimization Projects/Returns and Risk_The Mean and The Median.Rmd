...
The results of this project found that using the median return to find the optimal portfolio did not lead to significant improvement in portfolio performance than using the mean return. However, this should not be taken as evidence against the median return as the project was flawed in its own ways. Firstly, the portfolios optimized only used a selected number of stocks, which means that the results are not generalizable to any portfolio. Furthermore, I used random portfolios to simulate the performance of hypothetical portfolios and chose weights that optimized the return and/or risk measures during the training period and applied the chosen weights onto a return period. The random portfolios were only a small subset of an large possible set of permutations, decreasing the likelihood of finding the true optimal portfolio.
...
rp_returns <- foreach(i = opt_periods) %:%
  foreach(j = 1:nrow(rand_port), .combine = "cbind") %dopar% {
...
maxmed_weight <- foreach(i = 1:length(rp_returns), .combine = "rbind") %do% {
...
minstd_weight <- foreach(i = 1:length(rp_returns), .combine = "rbind") %do% {
...
minmad_weight <- foreach(i = 1:length(rp_returns), .combine = "rbind") %do% {
...
minmead_weight <- foreach(i = 1:length(rp_returns), .combine = "rbind") %do% {
  tmp <- apply(X = rp_returns[[i]], MARGIN = 2, FUN = function(x) {
    median(abs(x - median(x)))
    })
...
mean_port <- foreach(i = opt_periods) %do% {
  ...
  mean_target <- rand_port[which(tmp > as.vector(ewp_mean)),]
}
```
