...
| Specificity        | How well model was able to correctly predict negative cases, $TN/(TN+FP)$ (also called true negative rate |


## 5 Determining the Cut-off Probability

Rather than using an arbitrary cut-off probability, we can use the Receiver Operating Curve (ROC) to determine a better cut-off probability which can increase the true positives while lowering the false negatives. I would use Model 2 to illustrate the use of the in determining the threshold value.
