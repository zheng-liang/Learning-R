...
```{r load gridExtra package}
library(gridExtra)
```

```{r dist of sex_educ_marr_age}
...
Let us visualize the correlation between the variables *`BILLAMT_`* and *`PAYAMT_`*.
...
We can see that the correlation between *`BILLAMT_`* is very high, which is likely due to the time component involved in these variables. We could expect that if the bill statement in April is high, then the bill statement in May would likely be high as well. However, it seemed that the correlation between the *`PAYAMT_`* is rather moderate. For the correlation between *`BILLAMT_`* and *`PAYAMT_`*, there seems to be a pattern of relatively higher correlation on the diagonals, i.e. between *`BILLAMT_AUG`* and *`PAYAMT_SEP`* or *`BILLAMT_JUL`* and *`PAYAMT_AUG`*. We could expect that the bill statement in July was paid in August and so on. The high correlation between these variables would be an issue when adding them to the logit model and we might want to consider creating new features from these two variables.

### 4.1.3 Creating New Variables

The first variable I created is the average of each observation's *`BILLAMT_`* variables, divided by its *`LIMITT_BAL`*. This new variable would help us determine the average utilization of the credit given to a person.

```{r avg util}
avg_util <- round(rowMeans(billamt) / default.data$LIMIT_BAL, 4)
```

The next variable I created takes the average of each observations *`PAYAMT_`* variables, divided by the average of each observations *`BILLAMT_`* variables. This would help us determine how much of the bill statement was paid on average over April to September. However, the calculation would lead to `NaN` or `inf` values, which cannot be handled by the logistic regression formula, so I converted it to `NA` values (which can be handled).

```{r avg pmt bill ratio}
...
sum(is.nan(avg_pmt_bill_ratio)); sum(is.infinite(avg_pmt_bill_ratio)) # To check
```

The variables *`PAY_`* has too many categories, and so there was a need to reduce the number of values in this categorical variable. I have decided to only look at the variable *`PAY_SEP`* as it would be the most recent variable available. But before that, I would bind the variables that I need in the regression into a new data frame.

```{r bind variables}
new_data <- data.frame(default.data[,1:6], avg_util, avg_pmt_bill_ratio, default.data$default)

head(new_data, n = 4)
```

Looking at the variable *`PAY_SEP`* in a table format, I have reduced the original 11 classes into 2. 0 = those who paid duly/early or delayed payment for one month, 1 = those who delayed payment for two month or more.

```{r table default and paysep}
